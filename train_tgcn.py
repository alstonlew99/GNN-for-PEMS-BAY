import numpy as np, torch, h5py, pickle
...
with h5py.File("pems-bay-meta.h5", "r") as f:
    meta_coords = np.array(f["coordinates"])  # [N, 2]
    meta_ids = np.array(f["sensor_id"])

meta = {"coords": meta_coords, "ids": meta_ids}

with open("adj_mx_bay.pkl", "rb") as f:
    adj_precomputed = pickle.load(f)
...
# --- adjacency choice ---
ADJ_TYPE = "knn"       # or "precomputed" or "topo_flow"
K = 5
...
if ADJ_TYPE == "knn":
    ...
    A = build_topo_flow_adj(coords_xy=meta_coords, topo_mask=knn_mask,
                            node_mean=mean, node_var=var, node_cv=cv,
